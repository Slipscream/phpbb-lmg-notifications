...
class lmg_notifications_module
{
	...
	public function main($id, $mode)
	{
		...
		$this->tpl_name = 'acp_lmg_notifications';
	...
	private function generate_forum_section()
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			// Normal forums have a radio input with the value selected basd on the value of the lmg_notifications_enabled setting
			else if ($row['forum_type'] == FORUM_POST)
			{
				...
				$yes_checked = $row['lmg_notifications_enabled'] == 1 ? 'checked' : '';
